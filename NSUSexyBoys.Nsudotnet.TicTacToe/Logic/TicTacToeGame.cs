...
namespace Logic
{
    public class TicTacToeGame : ITicTacToeService
    {
        ...
        private void UpdateMainCondition()
        {
            ...
            for (var i = 0; i < 3; i++)
            {
                if ((mainCells[i, 0].Condition == mainCells[i, 1].Condition) &&
                    (mainCells[i, 1].Condition == mainCells[i, 2].Condition) && mainCells[i, 0].Condition != Condition.FREE)
                {
                    Field.Condition = mainCells[i, 0].Condition;
                }
                if ((mainCells[0, i].Condition == mainCells[1, i].Condition) &&
                    (mainCells[1, i].Condition == mainCells[2, i].Condition) && mainCells[0, i].Condition != Condition.FREE)
                {
                    Field.Condition = mainCells[0, i].Condition;
                }
            }
            if ((mainCells[0, 0].Condition == mainCells[1, 1].Condition) &&
                (mainCells[1, 1].Condition == mainCells[2, 2].Condition) && mainCells[0, 0].Condition != Condition.FREE)
            {
                Field.Condition = mainCells[0, 0].Condition;
            }
            if ((mainCells[2, 0].Condition == mainCells[1, 1].Condition) &&
                (mainCells[1, 1].Condition == mainCells[0, 2].Condition) && mainCells[2, 0].Condition != Condition.FREE)
            {
        ...
        private void UpdateSubCondition(int mainCol, int mainRow)
        {
            ...
            for (var i = 0; i < 3; i++)
            {
                if ((subCells[i, 0] == subCells[i, 1]) && (subCells[i, 1] == subCells[i, 2]) && subCells[i, 0] != Condition.FREE)
                {
                    subField.Condition = subCells[i, 0];
                    return;
                }
                if ((subCells[0, i] == subCells[1, i]) && (subCells[1, i] == subCells[2, i]) && subCells[0, i] != Condition.FREE)
                {
                    subField.Condition = subCells[0, i];
                    return;
                }
            }
            if ((subCells[0, 0] == subCells[1, 1]) && (subCells[1, 1] == subCells[2, 2]) && subCells[0, 0] != Condition.FREE)
            {
                subField.Condition = subCells[0, 0];
                return;
            }
            if ((subCells[2, 0] == subCells[1, 1]) && (subCells[1, 1] == subCells[0, 2]) && subCells[2, 0] != Condition.FREE)
            {
